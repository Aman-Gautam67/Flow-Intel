...
export function LanguageSwitcher() {
  ...
  useEffect(() => {
    ...
    window.addEventListener("flowintel-locale-preference-changed", sync);
    window.addEventListener("storage", sync);
    return () => {
      i18n.off("languageChanged", sync);
      window.removeEventListener("flowintel-locale-preference-changed", sync);
  ...
  return (
    <div className="flex items-center gap-1.5 rounded-full border border-border bg-background px-2 py-1 shadow-sm">
      ...
      <select
        ...
        className="max-w-[140px] cursor-pointer truncate text-xs font-medium outline-none appearance-none"
        style={{ background: "#0f0f0f", color: "#f0f0f0", colorScheme: "dark" }}
        title={
          preference === "system"
      ...
      >
        <option value="system" style={{ background: "#0f0f0f", color: "#f0f0f0" }}>{t("language.followSystem")}</option>
        <option value="en-US"  style={{ background: "#0f0f0f", color: "#f0f0f0" }}>{t("language.enUS")}</option>
        <option value="zh-CN"  style={{ background: "#0f0f0f", color: "#f0f0f0" }}>{t("language.zhCN")}</option>
      </select>
    </div>
  );
}
